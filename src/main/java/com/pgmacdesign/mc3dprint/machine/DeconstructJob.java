...
public final class DeconstructJob {
    ...
    public CompoundTag save() {
        ...
        tag.put("RegionSize", NbtUtils.writeBlockPos(size));
    ...
    public static DeconstructJob load(CompoundTag tag) {
        ...
        if (tag.contains("Unprint")) {
            placement = PrintPlacement.load(tag.getCompound("Unprint")).orElse(null);
            if (placement == null) {
                return null;
            }
        }
        DeconstructJob job = new DeconstructJob(
                tag.contains("Min") ? NbtUtils.readBlockPos(tag.getCompound("Min")) : BlockPos.ZERO,
                tag.contains("RegionSize")
                        ? NbtUtils.readBlockPos(tag.getCompound("RegionSize"))
                        : new BlockPos(1, 1, 1),
                placement);
        job.progress = Math.max(0, tag.getInt("Progress"));
        job.removed = tag.getInt("Removed");
        job.creditedFu = tag.getLong("CreditedFu");
        return job;
    }
}
