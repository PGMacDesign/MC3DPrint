...
public final class FuValueRegistry {
    ...
    private static synchronized Optional<FuValue> derive(Item item) {
        ...
        if (valuator == null) {
            MinecraftRecipeIndex index = new MinecraftRecipeIndex(boundRecipeManager, boundRegistryAccess);
    ...
    private static boolean parseEntry(String entry, Map<Item, FuValue> items, List<TagEntry> tags) {
        ...
        if ("off".equalsIgnoreCase(rhs)) {
            if (id.startsWith("#")) {
                ...
                items.remove(ForgeRegistries.ITEMS.getValue(parseItemId(id)));
            }
            return false;
        }
        ...
        if (id.startsWith("#")) {
            ...
            items.put(ForgeRegistries.ITEMS.getValue(parseItemId(id)), value);
        }
        return true;
    }
    ...
    public static List<String> defaultEntries() {
        return List.of(
                ...
                // netherite path that isn't ancient-debris mining. Both are manual grinds, never AFK
                // farms, so neither can pump: elytra is End-city exploration, notch apples are loot-
                // only (uncraftable, so pulled out of the blanket food blacklist for this). This is
                // what makes "convert equal-value rares into netherite" work through the winder
                // without touching the exact-tier backbone. (heavy_core is a 1.21+ item, absent here.)
