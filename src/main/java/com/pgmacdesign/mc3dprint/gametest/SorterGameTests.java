...
public class SorterGameTests {
    ...
    public static void cableAttachesToSorter(GameTestHelper helper) {
        // Guards the tag-directory trap: on 1.20.1 the tag lives in data/mc3dprint/tags/blocks/
        // (PLURAL — 1.21 renamed it to the singular "block/"). The wrong form for the version
        // silently fails to load, which would leave this tag empty and the cable visibly refusing
        // to attach — reading as a bug to the player.
        if (!ModBlocks.FILAMENT_ITEM_SORTER.get().defaultBlockState().is(ModBlockTags.CABLE_CONNECTABLE)) {
            helper.fail("sorter is not in mc3dprint:cable_connectable — check that"
                    + " data/mc3dprint/tags/blocks/ (plural) actually loaded");
            return;
        }
