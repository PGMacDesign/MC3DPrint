...
public class FuTierEconomyGameTests {
    ...
    @GameTest(template = "empty5", timeoutTicks = 100)
    public static void powderSnowPrintsFreeAsStructural(GameTestHelper helper) {
        // Powder snow prints FREE as structural matter so curated builds reproduce it exactly, and it
        // carries NO FU value — so it can't be printed-for-cost and can't be wound/laundered into FU.
        boolean structural = PrinterBlockEntity.isStructuralMatterForTest(
                net.minecraft.world.level.block.Blocks.POWDER_SNOW.defaultBlockState());
        boolean unvalued = com.pgmacdesign.mc3dprint.fu.FuValueRegistry.valueOf(
                new ItemStack(Items.POWDER_SNOW_BUCKET)).isEmpty();
        if (!structural) {
            helper.fail("Powder snow should print free as structural matter");
        } else if (!unvalued) {
            helper.fail("Powder snow must carry no FU value (free structural), but the bucket is valued");
        } else {
            helper.succeed();
        }
    }
    ...
    public static void powderSnowBucketIsNotWindable(GameTestHelper helper) {
        WinderBlockEntity winder = placePoweredWinder(helper, new BlockPos(2, 1, 2));
        // Powder snow carries no FU value (it prints free), so the winder must refuse the bucket even
        // into a same-tier spool — powder snow can never be laundered into FU.
        winder.inventory().setStackInSlot(WinderBlockEntity.SLOT_SPOOL, spoolWithFu(2, 0));
        ...
        helper.runAfterDelay(100, () -> {
            ...
            if (fu != 0) {
                helper.fail("Unvalued powder_snow_bucket was wound: spool gained " + fu + " FU");
            } else if (!inputKept) {
                helper.fail("Winder consumed an unwindable powder_snow_bucket");
            } else {
                helper.succeed();
            }
        });
