...
public class DeconstructGameTests {
    ...
    private static ItemStack sneakClickWithScanner(GameTestHelper helper, BlockPos target,
        ...
        tag.put(ScannerItem.TAG_CORNER_A, NbtUtils.writeBlockPos(helper.absolutePos(a)));
        tag.put(ScannerItem.TAG_CORNER_B, NbtUtils.writeBlockPos(helper.absolutePos(b)));
    ...
    public static void scannerArmsDeconstructRegionFromAnyCasing(GameTestHelper helper) {
        ...
        Optional<BlockPos> keptA = corner(scanner, ScannerItem.TAG_CORNER_A);
        Optional<BlockPos> keptB = corner(scanner, ScannerItem.TAG_CORNER_B);
        if (!keptA.equals(Optional.of(helper.absolutePos(a)))
                || !keptB.equals(Optional.of(helper.absolutePos(b)))) {
            helper.fail("scanner corners were modified by the hand-off: " + keptA + "/" + keptB);
            return;
        }
        helper.succeed();
    }

    private static Optional<BlockPos> corner(ItemStack scanner, String key) {
    ...
    public static void unformedCasingStillSetsAScannerCorner(GameTestHelper helper) {
        ...
        Optional<BlockPos> cornerA = corner(scanner, ScannerItem.TAG_CORNER_A);
        if (!cornerA.equals(Optional.of(helper.absolutePos(casing)))) {
            helper.fail("expected corner A at the clicked casing, got " + cornerA);
            return;
        }
        helper.succeed();
    }
    ...
    public static void unreadableArmedUnprintClearsTheRegion(GameTestHelper helper) {
        ...
        CompoundTag saved = printer.saveWithoutMetadata();
        ...
        saved.put("ArmedUnprint", new CompoundTag());
