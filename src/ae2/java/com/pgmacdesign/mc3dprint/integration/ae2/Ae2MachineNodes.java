 ...
 * compiled on versions with no AE2 at all. {@link MachineAttachments} is the seam.
 */
final class Ae2MachineNodes implements MachineAttachments.Attachment {
    ...
    static void install(RegisterCapabilitiesEvent event) {
        Ae2MachineNodes nodes = new Ae2MachineNodes();
        MachineAttachments.register(nodes);
        // Context is Void for this capability: a node host answers per-side through getGridNode
        // rather than being looked up per-side.
        event.registerBlockEntity(AECapabilities.IN_WORLD_GRID_NODE_HOST,
                ModBlockEntities.PRINTER.get(),
                (be, context) -> NODES.get(keyOf(be)));
    }
