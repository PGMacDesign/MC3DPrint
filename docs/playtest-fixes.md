...
- [ ] **iron_farm — zombie cell** — the east-wall zombie cell is a 1-wide slot open at deck level (sides only walled y=11+), so a dropped zombie steps out onto the spawn deck and walks into the central drop hole. Fix: a FULLY ENCLOSED 1×2 glass box (walled y=10..12 on every deck-facing side, capped by the y=13 roof) — contained, fits a boat, villagers keep line-of-sight.
- [ ] **iron_farm — wash water floods the room** — the spawn-deck water pours down the 2×2 drop shaft and floods the collection room. Fix: a 2×2 layer of **wall signs** in the drop mouth (y=3, attached to the surrounding ceiling brick) to dam the water (signs block water flow; golems/items fall through), keeping the room dry.
